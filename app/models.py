...
class RegulationEnum(enum.Enum):
    customer_limit = "Giới hạn khách hàng trong 1 phòng"
    customer_extras = "Phụ thu cho khách hàng cuối cùng"
    foreigner_extras = "Phụ thu cho khách hàng nước ngoài"
...
class LoaiPhong(db.Model):
    ...
    # cac_phong = relationship("Phong", backref="loai_phong")
    cac_phong = relationship("Phong", back_populates="loaiphong", lazy=True)

    # def __repr__(self):
    #     return '<LoaiPhong %r>' % self.loai_phong
...
class Phong(db.Model):
    id = Column(String(10), primary_key=True, nullable=False, autoincrement=False)
    tinh_trang = Column(BOOLEAN, nullable=False, default=True)
    id_loaiphong = Column(String(45), ForeignKey("loai_phong.id"), nullable=False)
    # phong_thoigian = relationship("ThoiGianTraThuePhong", backref="phong", lazy=True)
    loaiphong = relationship("LoaiPhong")
...
class PhieuDatThuePhong(db.Model):
    ...
    id_user = Column(Integer, ForeignKey(User.id), nullable=False) # booker
    thoi_gian_dat = Column(DATETIME, nullable=False)
    cho_thue = Column(BOOLEAN, nullable=False, default=False)
    phieudat_thoigian = relationship("ThoiGianTraThuePhong", backref="PhieuDatThuePhong", lazy=True)
    phieudat_hoadon = relationship("HoaDon", backref="PhieuDatThuePhong", lazy=True)


class ThoiGianTraThuePhong(db.Model):
    id_khachhang = Column(Integer, ForeignKey(KhachHang.id), nullable=False, primary_key=True)
    id_phong = Column(String(10), primary_key=True, nullable=False)
...
class QuyDinh(db.Model):
    regulation = Column(Enum(RegulationEnum), primary_key=True)
...
if __name__ == "__main__":
    with app.app_context():
        ...
        us1 = KhachHang(ho="Trần Thanh", ten="Hoàng", ngay_sinh=time.date(), cccd="07228311",
                        user_role=UserRole.khach_hang)
        time = datetime.datetime(year=2023, month=9, day=2)
        us2 = LeTan(ho="Vương Minh", ten="Khánh", ngay_sinh=time.date(), cccd="07920302", luong=5000,
                    user_role=UserRole.le_tan)
        time = datetime.datetime(year=2003, month=1, day=1)
        us3 = KhachHang(ho="Huỳnh Duy", ten="Đông", ngay_sinh=time.date(), cccd="0123456",
                        user_role=UserRole.khach_hang)
        ...
        db.session.commit()
        time = datetime.datetime(year=2023, month=9, day=2)
        lt = LeTan(ho="Lễ", ten="Tên", ngay_sinh=time.date(), cccd="073424243", luong=5000, user_role=UserRole.le_tan,
                   username="nv1", password=hashlib.md5('123'.encode('utf-8')).hexdigest())
        db.session.add(lt)
        db.session.commit()
        ...
        rent1 = ThoiGianTraThuePhong(id_khachhang=2, id_phong="103", thoi_gian_thue=cki, thoi_gian_tra=cko,
                                     id_datphong=1)
        ...
        rent2 = ThoiGianTraThuePhong(id_khachhang=2, id_phong="301", thoi_gian_thue=cki, thoi_gian_tra=cko,
                                     id_datphong=1)
        ...
        rent3 = ThoiGianTraThuePhong(id_khachhang=4, id_phong="301", thoi_gian_thue=cki, thoi_gian_tra=cko,
                                     id_datphong=2)
        db.session.add_all([rent1, rent2, rent3])
        db.session.commit()

        reg1 = QuyDinh(regulation=RegulationEnum.customer_limit, value=3)
        reg2 = QuyDinh(regulation=RegulationEnum.customer_extras, value=0.25)
        reg3 = QuyDinh(regulation=RegulationEnum.foreigner_extras, value=1.5)
